...
ETS models use multiplicative errors and multiplicative seasonality for handling time series with variance which increases with the level of the series. While ETS models are based on the trend and seasonality, ARIMA models describe the autocorrelations in the data.
...
- If a time series is stationary, then the ACF will drop to zero quickly
- If a time series is non-stationary, then the ACF will drop to zero slowly

> ***Differencing can be applied as many times as necessary to achieve stationarity.***
...
If your time series data has a seasonal pattern, then additional seasonal components (P,D,Q) are added to the model. The seasonal components are similar to the non-seasonal components but they are *backshifts* of the seasonal period. For example, if you wanted to forecast sales for some product in February, you might predict that based on last February's sales. Click [here](https://online.stat.psu.edu/stat510/lesson/4/4.1) to learn more about seasonal ARIMA models. 

---

# 2. Transformations for Variance Stabilization

There are multiple ways to change time series data so that it becomes stationary. Some time series data show increasing variation as the level increases. In these cases, transforming the data to reduce the variance can be helpful. 
...
The following examples will utilize a dataset containing the total monthly generation of electricity in kWh from 1973-2013 in the United States.  
...
> ***Note that the size of the seasonal fluctuations increases as the level increases.***
...
> ***Note how the Box-Cox transformation stabilizes the variance as the level increases.***
...
The following examples will use a `fpp2` dataset containing the annual female murder rate per 100,000 people in the United States from 1950-2004. 

```{r}
# Plot female murder rate
m1 <- autoplot(wmurders) + 
...
> ***Does the differenced data look stationary? Does the ACF plot of the differenced data show significant autocorrelation left?***
...
> ***The differenced data shows clear stationarity. The ACF & PACF plots of the differenced data shows very little autocorrelation as well.***
...
- First differences are the change between an observation and the next observation.
- Seasonal differences are the change between one year to the next year.

The following examples will utilize a dataset containing the monthly corticosteroid expenditure in millions of dollars in Australia from 1991-2008. 
...
> ***This non-stationary time series would benefit from applying a log transformation before differencing.*** 
...
> ***One handy feature of `modeltime` and `timetk` is the use of a LOESS smoother which helps in determining the stationarity of a series.***
...
All three forecasting frameworks rely on the Hyndman-Khandakar algorithm to automatically choose an ARIMA model. The algorithm uses a combination of unit root tests, minimization of the AICc score, as well as the minimization of the maximum likelihood estimation to automatically choose the best model. However, it is important to know that it uses approximations and a stepwise approach to speed up the search for a model by default. It's possible to search a much larger set of ARIMA models if you do not use either methods. Here's a general procedure for automatically choosing an ARIMA model for non-seasonal data: 
...
> ***The first plot shows the original trade index series which is non-stationary as it shows a clear upwards then downwards trend. The second plot shows that the first seasonally-differenced annual change in index series is still non-stationary showing an overall downward trend. The third plot shows that the seasonally-differenced and first-differenced change in index series is finally stationary. Let's apply two different automatically chosen ARIMA models.***
...
fit_eu_retail_011_011 %>% 
  gg_tsresiduals() + 
  ggtitle("Residuals from fitted ARIMA(0,1,1)(0,1,1) model")
```
...
ARIMA and ETS models are some of the most widely used forecasting techniques. They are complementary to each other: ARIMA models focus on the autocorrelations in the data whereas ETS models focus on the trend and seasonality. Every forecaster should consider trying ETS and ARIMA models before jumping to more complex methods like machine-learning and neural networks. 

Here are some of my thoughts about ARIMA models and the three forecasting frameworks utilized in this article:
...
- The `fpp2` and `fpp3` frameworks focus more on minimizing the AICc for ARIMA model selection whereas `modeltime`+`timetk` focuses more on accuracy measures on a test set.
